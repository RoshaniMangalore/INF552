#Authors: Kavya Sethuraman, Rasika Guru, Roshani Mangalore
...
def train_function(train_data, train_label):
    ...
    while (epochs):
        for i in range(train_data.shape[0]):
            ...
            deltal = deltaEX*deltaXS
            gradient = (learning_rate * deltal)* X2
            for i in range(100):
            ...
            deltalminus1= sum(deltal*hidden_weights.T)*(der_sigmoid(S1))

            input_weights = input_weights - learning_rate*X1.reshape([960, 1])*(deltalminus1.reshape([1, 100]))
...
#Function to Test Neural Network using the updated weights which are outputted by Train function
def test_function(test_data, test_label):
    ...
    for i in range(len(prediction)):
        if int(prediction[i]) == int(test_label[i]):
            count = count + 1
    print float(count / len(prediction)) * 100
...
#Uncomment the train function to train the Neural Network
#train_function(train_data, train_label)

#functioncall to test the Neural Network Prediction
